...
describe("Stake and claim rewards", () => {
  ...
  it("Create Reward Distributor", async () => {
    ...
    expect(rewardDistributorData.parsed.defaultMultiplier.toNumber()).to.eq(0);
  ...
  it("Create Stake And Reward Entry", async () => {
    ...
    const [rewardDistributorId] = await findRewardDistributorId(stakePoolId);
    const oldRewardDitributor = await getRewardDistributor(
      provider.connection,
      rewardDistributorId
    );
    expect(oldRewardDitributor.parsed.defaultMultiplier.toNumber()).to.eq(0);
    ...
    const newRewardDitributor = await getRewardDistributor(
      provider.connection,
      rewardDistributorId
    );
    expect(newRewardDitributor.parsed.defaultMultiplier.toNumber()).to.eq(1);
  });
});
